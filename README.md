...
go run ./server/cmd/openrtc dev probe --restart runtime --realtime --yjs-realtime
...
- `openrtc dev probe` runs the same endpoint checks from a terminal or CI job, including optional runtime/admin restart drills, JSON output, `--realtime` for a tokenized runtime WebSocket join plus sequenced storage patch, and `--yjs-realtime` for a live Yjs WebSocket update check.
